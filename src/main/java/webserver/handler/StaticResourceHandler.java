...
public class StaticResourceHandler extends HttpRequestHandler {
    ...
    public HttpResponse handleResponse(HttpRequest httpRequest) {
        ...
        HttpRequestUri httpRequestUri = httpRequest.header().uri();
        try (InputStream is = getClass().getClassLoader().getResourceAsStream(httpRequestUri.resourcePath())) {
            // 요청에 맞는 리소스가 존재하지 않음
            if (is == null) {
                throw new ResourceNotFoundException("Resource not found: " + httpRequestUri);
            }
            ...
            return new HttpResponse(
                HttpResponseHeader.builder()
                    ...
                    .contentType(httpRequest.header().uri().contentType())
                    .body(body)
                    .build(),
                body);
